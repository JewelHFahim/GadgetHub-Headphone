...
const Feature = () => {
  ...
  return (
    <div className="px-[20px] lg:px-[16vw] pt-[30px] pb-[20px] lg:mt-[75px]">
      <div className="flex justify-between items-start lg:items-center">
        <div>
          <SubTitle
            className={`text-[40px] lg:text-[60px] ${
              isDarkmode ? "text-white" : "text-bgPrimary"
            }`}
          >
            OUR
          </SubTitle>
          ...
          <PrimaryTitle
            className={`text-[50px] lg:text-[120px] font-poppins leading-[40px] lg:leading-[100px] ml-[-4px] ${
          ...
          <div className="relative w-[25px] h-[25px] lg:w-[44px] lg:h-[44px]">
            {isDarkmode ? (
              <Image
                ...
                fill={true}
                alt=""
                className={`p-2 rounded-full bg-[#191919]`}
              />
            ) : (
              <Image
                src="/rightIconDark.png"
                fill={true}
                alt=""
                className={`p-2 rounded-full bg-bgOffWhite`}
              />
            )}
          </div>
          <p
            className={`text-[10px]  lg:text-[15px] font-poppins  ${
      ...
      <div className="hidden mt-[12px] lg:grid grid-cols-1 lg:grid-cols-3">
        {datas.map((item, index) => (
          <div
            key={index}
            className={`w-[21.5vw] h-[100%] border border-opacity-[60%] p-[8%] ${
              isDarkmode ? "border-white" : "border-black"
            } transform duration-300 hover:scale-[1.03]`}
            style={{ marginTop: `${item.position}px` }}
          >
            <div className="flex items-center gap-[25px]">
              <Image src={item.icon} alt="" width={85} height={80} />
              <h2
                className={`text-[30px] font-[800] font-poppins ${
            ...
            <p
              className={`mt-[58px] text-[12px] font-[300] ${
                isDarkmode ? "text-white text-opacity-[50%]" : "text-black"
              }`}
            >
              {item.desc}
            </p>

            <button className="flex items-center gap-[10px] mt-[4vh]">
              <div className="relative w-[25px] h-[25px] lg:w-[44px] lg:h-[44px]">
                {isDarkmode ? (
                  <Image
                    src="/rightIcon.png"
